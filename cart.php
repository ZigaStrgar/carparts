    ...
    <div class="stepwizard">
        <div class="stepwizard-row">
            <div class="stepwizard-step">
                <button type="button" class="btn btn-primary btn-circle"><i class="icon icon-shopping-cart"></i>
                </button>
                <p>Košarica</p>
            </div>
            <div class="stepwizard-step">
                <button type="button" class="btn btn-default btn-circle" disabled="disabled"><i
                        class="icon icon-checklist"></i></button>
                <p>Pregled</p>
            </div>
            <div class="stepwizard-step">
                <button type="button" class="btn btn-default btn-circle" disabled="disabled"><i
                        class="icon icon-credit-card-2"></i></button>
                <p>Konec</p>
            </div>
        </div>
    </div>
        ...
        <table class="table table-bordered table-striped table-hover col-lg-12">
            ...
            <?php if (Db::query("SELECT *, s.pieces AS spieces, p.pieces AS parts, s.id AS oid FROM cart s INNER JOIN parts p ON p.id = s.part_id WHERE s.user_id = ?", $_SESSION["user_id"]) > 0) { ?>
                <?php foreach ($cart_offers as $offer) { ?>
                    ...
                    ?>>
                        <td><a href="http://<?php echo URL; ?>/part/<?php echo $offer["pid"] ?>"
                               target="_blank"><?php echo $offer["name"]; ?></a></td>
                        <td><input class="form-control" type="text" name="pieces"
                                   data-offer-max="<?php echo $offer["parts"] ?>"
                                   data-offer-id="<?php echo $offer["oid"] ?>" value="<?php echo $offer["spieces"] ?>"
                                   placeholder="Vnesite št. kosov"/></td>
                        <td><?php echo $offer["parts"] ?></td>
                        <td><?php echo price($offer["price"]) ?> €</td>
                        <td><i class="icon icon-remove color-danger pull-right" style="cursor: pointer;"
                               onClick="removeOffer(<?php echo $offer["oid"] ?>)" data-placement="left"
                               data-toggle="popover" data-content="Odstrani iz košarice"></i></td>
                    </tr>
                <?php } ?>
                <tr>
