...
if (!empty($min) && !empty($max)) {
    $searchQuery = "SELECT *, p.id AS pid, p.name AS pname FROM parts p INNER JOIN models_parts mp ON mp.part_id = p.id INNER JOIN models m ON m.id = mp.model_id WHERE p.deleted = 0 AND p.pieces > 0 AND p.price >= $min AND p.price <= $max ";
} else {
...
if (!empty($_GET["type"])) {
    ...
    $searchQuery = "SELECT *, p.id AS pid, p.name AS pname FROM parts p INNER JOIN models_parts mp ON mp.part_id = p.id WHERE p.deleted = 0 AND p.pieces > 0 AND lower(mp.type) LIKE '%$type%'";
}
...
//Poglej za kataloško številko
    $resultNumber = Db::queryAll("SELECT *, id AS pid, name AS pname FROM parts WHERE deleted = 0 AND pieces > 0 AND number = ?", $number);
}
//ZA PONOVNO ISKANJE
//TIPI
